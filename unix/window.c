...
struct GtkFrontend {
    ...
    GtkWidget *menu, *specialsmenu, *specialsitem1, *specialsitem2,
        *restartitem, *disconnectitem, *alwaysontopitem;
...
static void key_pressed(GtkFrontend *inst)
{
     ...
     */
    if (inst->exited && !inst->user_disconnected)
        gtk_widget_destroy(inst->window);
}
...
static void destroy_inst_connection(GtkFrontend *inst)
{
    ...
    if (inst->menu) {
        ...
        if (inst->disconnectitem)
            gtk_widget_set_sensitive(inst->disconnectitem, false);
    }
}
...
void disconnect_session_menuitem(GtkMenuItem *item, gpointer data)
{
    GtkFrontend *inst = (GtkFrontend *)data;
    ...
    if (inst->backend) {
        ...
        logevent(inst->logctx, "----- Session disconnected by user -----");
        inst->user_disconnected = true;
        destroy_inst_connection(inst);
    }
}
...
void app_menu_action(GtkFrontend *frontend, enum MenuAction action)
{
    ...
    switch (action) {
      ...
      case MA_RESTART_SESSION:
        ...
        break;
      case MA_DISCONNECT_SESSION:
        disconnect_session_menuitem(NULL, inst);
        break;
...
static void start_backend(GtkFrontend *inst)
{
    ...
    if (inst->disconnectitem)
        gtk_widget_set_sensitive(inst->disconnectitem, true);
}
...
#define MKCHECKITEM(title, func) do                                     \
        ...
        MKMENUITEM("Disconnect", disconnect_session_menuitem);
        inst->disconnectitem = menuitem;
